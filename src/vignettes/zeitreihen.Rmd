...
## Appendix
| Funktion      | Argumente              | Ausgabewerte                                         | Beschreibung / Verwendung                              |
|---------------|------------------------|------------------------------------------------------|--------------------------------------------------------|
| `make_AR`     | `t, phi, sigma, start` | Zeitreihe (`X`)                                      | Erzeugt eine AR(p)-Zeitreihe                           |
| `make_MA`     | `t, theta, sigma`      | Zeitreihe (`X`)                                      | Erzeugt eine MA(q)-Zeitreihe                           |
| `sample_ACVF` | `X, h`                 | Autokovarianz (`gamma`)                              | Berechnet Autokovarianz für verschiedene Lags          |
| `periodogram` | `X, lambda`            | Periodogramm (`I_n`)                                 | Schätzt spektrale Dichte einer Zeitreihe               |
| `DLA`         | `X`                    | AR-Koeffizienten (`phi`), Innovationsvarianzen (`v`) | Berechnet Koeffizienten für ein                        |
| `IA`          | `X, max_lag`           | Innovationskoeffizienten (`theta`), Varianzen (`nu`) | Berechnet Koeffizienten und Varianzen für ARMA-Modelle |
| `plot`        | `X, h`                 | - (Erzeugt Plot)                                     | Visualisiert Zeitreihe                                 |
...
- [Introduction to Time Series and Forecasting](https://link.springer.com/book/10.1007/978-3-319-29854-2)
  ...
  - sample autocovariance function: **3.2.1 Calculation of the ACVF**
