...
model(m)
```
We use the `DLA` function from the `zeitreihen` package to calculate the coefficients `phi` and the mean squared error `nu` for this time series, focusing on `m = 2`. The `DLA` function returns an anonymous function that takes `m` as a parameter, where `m` represents the amount last values used to predict the next value of the time series using the Durbin-Levinson algorithm. This function calculates the necessary coefficients and mean squared errors, returning a list with two components: `phi` (the coefficients) and `nu` (the mean squared error).
...
| `DLA`              | `X`, `m`                     | model coefficients (`phi`), error of the one-step predictors (`nu`)             | calculates the coefficients for a given model   |
| `IA`               | `X`, `max_lag`               | model coefficients (`coeffs`), innovations (`nu`), coefficient matrix (`theta`) | calculates the coefficients for a given model   |
