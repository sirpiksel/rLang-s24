...
|--------------------|------------------------|------------------------------------------------------|-------------------------------------------------|
| `make_AR`          | `t, phi, sigma, start` | time series (`X`)                                    | Erzeugt eine AR(p) time series                  |
| `make_MA`          | `t, theta, sigma`      | time series (`X`)                                    | Erzeugt eine MA(q) time series                  |
| `sample_ACVF`      | `X, h`                 | Autocovariance (`gamma`)                             | calculates the autocovariance for lags          |
| `periodogram`      | `X, lambda`            | Periodogram (`I_n`)                                  | estimates the spectral density of a time series |
| `DLA`              | `X`                    | AR-Coefficients (`phi`), Innovations-variances (`v`) | calculates the coefficients for a given model   |
| `IA`               | `X, max_lag`           | Innovations-coefficients (`theta`), Variances (`nu`) | calculates the coefficients for a given model   |
