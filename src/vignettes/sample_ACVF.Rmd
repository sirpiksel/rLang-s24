...
* For each $h_i$ $\in$ `h`: we compute $n^{-1} \sum_{t=1}^{n - |h_i|} (x_{t + |h_i|} - \bar{x}) \cdot (x_t - \bar{x})$
  * We considered the vector, which is stores the last $n$ - `h` values from `X`, and subtract component wise the mean of `X` form it.
  * We considered the vector, which is stores the first $n$ - `h` values from `X`, and subtract component wise the mean of `X` form it.
