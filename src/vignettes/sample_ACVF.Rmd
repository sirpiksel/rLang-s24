...
* If the output value is near zero then we can assume that there is weak relation between the points $X_t$ and $X_{t + h}$. 
* If the output value is big and positive then we can assume that there is a strong relation between the points $X_t$ and $X_{t + h}$. 
* If the output value is big and negative then we can assume that there is a strong relation between the points $X_{t + h}$ and $X_{t}$.
...
The Output is the value `-0.8`, which is the computation of the `sample_ACVF` on `X` for `h`, and has named attribute `2`, which is the corresponding lag `h`. 

What happens:

* Within the function, It is checked if `X` and `h` have the required format.
* The mean of `X`, which is $\bar{x}$, is computed.
* The length of `X`, which is $n$, is extracted from `X`.
* For each $h_i$ $\in$ `h`: we compute $n^{-1} \sum_{t=1}^{n - |h_i|} (x_{t + |h_i|} - \bar{x}) \cdot (x_t - \bar{x})$
  * We considered the vector, which is formed by the last $n$ - `h` values from `X`, and subtract component wise the mean of `X` form it.
  * We considered the vector, which is formed by the first $n$ - `h` values from `X`, and subtract component wise the mean of `X` form it.
